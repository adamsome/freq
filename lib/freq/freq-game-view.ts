...
import { Guess, Header, Player, PlayerWithGuess } from '../../types/game.types'
...
import { getNextPsychic, getPsychic } from './freq-game'
import {
  getDirectionCounts,
...
function canChangePsychicTo(
  phase: FreqPhase
): FreqGameView['canChangePsychicTo'] {
  switch (phase) {
    case 'prep':
    case 'win':
      return 'any'
...
function showTarget(game: FreqGame, currentPlayer?: Player): boolean {
  ...
  if (isFreeFreqPhase(game.phase)) return true
  return false
}
...
function createCommands(
  ...
  const cmd: FreqCommand = { text: '' }
  const view: FreqCommandsView = { headers: [header], commands: [cmd] }
  ...
  switch (game.phase) {
    ...
    case 'direction': {
      ...
      const lockCmd: FreqCommand = { text: lockText }
